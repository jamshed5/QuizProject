...
// menuDiv -> create Buttons inside menuDiv
const buttons = [
...
const validationResetColor = () => {
  document
    .querySelectorAll("#option1, #option2, #option3, #option4")
    .forEach((input) => {
...
const updateOptionColors = (correctAnswer) => {
  ...
  options.forEach((input) => {
    ...
    if (value === correctAnswer) {
      input.style.backgroundColor = "#90EE90"; // light green for correct answer
    } else if (value !== "") {
      input.style.backgroundColor = "#FF7F7F"; // light red for incorrect options
    } else {
...
const getWinner = (players) => {
  ...
  return winners.length > 1
    ? "It's a tie!"
    : `The winner is ${winners[0].playerName} with ${winners[0].score} points!`;
};
...
const renderQuiz = (player, callback) => {
  ...
  quizArrayOfObjects.forEach((quiz, index) => {
    ...
    quiz.options.forEach((option) => {
      ...
      label.setAttribute("for", `quiz-${index}`);
      label.setAttribute("id", `quiz-${index}`);
...
const addPlayersAndRunCompetition = () => {
  ...
  addPlayerbtn.addEventListener("click", () => {
    ...
    form.appendChild(submitPlayer);
    playersForm.appendChild(form);
    ...
    form.addEventListener("submit", (event) => {
      ...
      const nextPlayer = () => {
        ...
        if (currentPlayerIndex > 0) {
          ...
          previousPlayer.innerHTML = `<h3>Previous Player: ${
            players[currentPlayerIndex - 1].playerName
          }</h3>`;
  ...
  inputField.addEventListener("input", (event) => {
    ...
    quizArrayOfObjects.forEach((quiz) => {
      if (quiz.question.toLowerCase().includes(query)) {
        ...
        const highlightedQuestion = quiz.question.replace(
          new RegExp(query, "gi"),
